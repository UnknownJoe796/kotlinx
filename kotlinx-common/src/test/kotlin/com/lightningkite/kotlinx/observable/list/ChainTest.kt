...
//                println(it.source.joinToString(transform = Char::toString))
                assertTrue(it.transformed.toSet().sorted() deepEquals it.transformer.invoke(it.source).toSet().sorted())
                assertTrue(it.transformed.size.toString() == it.transformer.invoke(it.source).size.toString())
            }
