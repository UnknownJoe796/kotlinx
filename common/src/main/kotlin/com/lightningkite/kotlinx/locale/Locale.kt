...
data class TimeStamp(val millisecondsSinceEpoch: Long) {
    constructor(date: Date, time: Time, offset: Long = KotlinTime.defaultLocale.getTimeOffset()) : this(
            leap(
    ...
    fun date(offset: Long = KotlinTime.defaultLocale.getTimeOffset()): Date = Date((unleap(millisecondsSinceEpoch - offset) / TimeConstants.MS_PER_DAY).toInt())
    fun time(offset: Long = KotlinTime.defaultLocale.getTimeOffset()): Time = Time((unleap(millisecondsSinceEpoch - offset) % TimeConstants.MS_PER_DAY).toInt())
}

private fun unleap(millisecondsSinceEpoch: Long): Long = millisecondsSinceEpoch - LeapSeconds.first { millisecondsSinceEpoch > it.first }.second
private fun leap(millisecondsSinceEpoch: Long): Long = millisecondsSinceEpoch + LeapSeconds.first { millisecondsSinceEpoch > it.first }.second
