    ...
    fun testOnChange() {
        ...
        for (i in testData.source.indices) {
            ...
            val callback = { old: Char, new: Char, index: Int ->
                ...
                assertTrue(validIndices.contains(index), "Index $index is not a valid index.  Valid indices are $validIndices")
            }
            testData.transformed.onChange += callback
            testData.source[i] = newElement
            assertTrue(testData.transformed.onChange.remove(callback))
        }
    }
}